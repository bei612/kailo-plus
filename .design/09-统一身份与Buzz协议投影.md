...
- 自动 Tenant onboarding 不从 IdP claim 推导；首次已认证 Human 只有在已有 invitation/provisioning fact 时建立 TenantMembership，否则保持 authenticated-but-unassigned。invitation 的寻址、接受、过期与撤回尚无定义，受 GAP-IDN-01 阻断；一期唯一的 provisioning fact 是 DD-82 的部署引导，它建立 Tenant 与首位 Tenant admin。
