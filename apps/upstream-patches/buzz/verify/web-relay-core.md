...
## 媒体读写（`DD-39`：Relay media 亦由 BFF 以 actor identity 签名读写）

Blossom kind 24242 的授权事件由 Core 代签——Browser 不持 signer，这份授权它签不出来。

两处收紧：

- **路径不接受调用方拼。** 取回端点只收 64 位十六进制的 sha256，由 Core 拼成
  `/media/<sha256>`；客户端库再额外拒绝 `/media/` 之外的路径与 `..`。放开它等于
  把 Relay 的整个 HTTP 面变成一个可代签的代理。
- **大小上界在 BFF 侧先行设定**（`07` §5），不把压力透传给 Relay。超限是确定的
  拒绝，而不是让上游给一个不区分成因的错误。

授权的有效期由 Core 给死一个短窗口，不让调用方指定：这份授权由 Core 代签，
调用方没有理由决定它能用多久。

编码用 base64url 无填充（BUD-11 规定的形式）。上游同时接受标准 base64 以兼容
nostr-tools，但兼容分支不是我们该依赖的东西。

`x` 标签与 `X-SHA-256` 头由同一次计算得出：上游按头查 `x` 标签，对不上即拒；
两处各算一次会在实现漂移时给出一个不区分成因的 400。

实测：上传拿到 blob descriptor；按 sha256 取回的字节**逐字节等于**上传的
（Core 只代签与转发，不改内容）；非法 sha256 被拒；撤权后上传被拒。
