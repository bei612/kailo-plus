...
- **签名**：`cosign` 对每个镜像 digest 签名，**在出现共享 registry 时启用**。当前无远端、无第二参与方、单机构建，本地密钥对的人工管理成本换不到任何可验证收益，因此 Stage 0 只做 digest、SBOM 与 provenance 三项（`00-实施总纲.md` Stage 0 第 7 项的字面要求）。接入远端并具备 OIDC 身份后直接启用 keyless 签名，跳过本地密钥阶段。签名启用后，部署前校验签名，验签失败拒绝启动。
