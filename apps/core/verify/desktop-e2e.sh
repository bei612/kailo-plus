#!/usr/bin/env bash
# Buzz Desktop 的 Kailo 接入端到端核验：以桌面端自己的 Rust 代码登录 IdP、登记
# 设备公钥、取 Community 连接事实、以设备私钥直连 Relay 发消息，并核验治理与撤销。
#
# Workspace 由 verify_workspace 夹具经真实 lifecycle Workflow 开通，成员是 IdP 里
# 那个真能登录的核验用户。Community host 在本机没有 DNS：演练期间在 /etc/hosts
# 临时加一行，结束时删除。
#
# 用法（apps 根目录）：core/verify/desktop-e2e.sh <已打补丁的 buzz 源码树>
set -euo pipefail
cd "$(dirname "$0")/../.."
. core/verify/integration-env.sh
src="$(realpath "${1:?用法: desktop-e2e.sh <已打补丁的 buzz 源码树>}")"
[ -d "$src/desktop/src-tauri" ] || { echo "$src 不是 buzz 源码树" >&2; exit 2; }
...
tenant="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["tenant"])' "$work/workspace.json")"
...
echo "Workspace 已开通，Community $host"
...
KAILO_E2E_CONVERGE_SECS="$VERIFY_CONVERGE_BOUND_SECS" \
  cargo test --lib kailo::e2e -- --ignored --nocapture
