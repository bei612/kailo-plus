...
/// 部署引导（ADR-11）出来的 Tenant 上的夹具：执行引导、开通额外成员、拆除。
/// 角色管理与审批续跑的核验共用一份。
pub mod bootstrapped {
    ...
    pub fn bootstrap(slug: &str, subject: &str, wait: u64) -> (i32, Value) {
    ...
    pub async fn add_member(
        ...
        let provider: Uuid = sqlx::query_scalar(
            "select id from identity.identity_provider where issuer = $1 limit 1",
        )
        .bind(&e.oidc_issuer)
        .fetch_one(pool)
        .await
        ...
        .await
        .map_err(|e| e.to_string())?;
        let action = super::allowed_action(pool, tenant, initiator, m.membership).await?;
    ...
    pub fn subj(p: Uuid) -> String {
        format!("principal:{p}")
    }

    /// 拆除引导出的 Tenant：先归档 Community、删 SpiceDB 关系，再按外键逆序删行。
    pub async fn teardown(e: &Env, pool: &PgPool, tenant: Uuid, humans: &[Uuid]) {
        let retired = super::retire_relay_community(e, pool, tenant).await;
        ...
        for p in &principals {
            for rel in ["member", "admin"] {
                super::zed_relationship(e, "delete", &format!("tenant:{tenant}"), rel, &subj(*p));
                for ws in &workspaces {
                    super::zed_relationship(
                        e,
                        "delete",
                        &format!("workspace:{ws}"),
                        rel,
