...
8. Browser 不持有 Nostr 私钥、平台 service credential、SpiceDB/Temporal/OpenMeter/AgentGateway 管理凭据，也不直连 Buzz Relay 或应用组件管理 API。
...
12. 前端只使用 Buzz Web 的主题、i18n、Host API 与语义命令；权限判断永远在服务端重做。
13. 每个变更必须有最小自动化验证：单元测试、合同测试或端到端场景；修复缺陷时先提交能复现失败的测试。
...
20. 完成声明前必须实际运行任务范围内的格式、静态检查、测试与追溯检查，并报告原始结果。
